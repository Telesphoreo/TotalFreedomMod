    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent event)
    {
        if (!event.getAuthor().getId().equals(Discord.bot.getSelfUser().getId()))
        {
            ...
            if (event.getMessage().getContentRaw().matches("[0-9][0-9][0-9][0-9][0-9]"))
            {
                ...
                event.getChannel().sendMessage("Link successful! This Discord account is linked with your Minecraft account **" + name + "**.\n"
                        + "When you are an impostor on the server, you may use `/verify` to verify.").complete();
            }
        }
    }
}